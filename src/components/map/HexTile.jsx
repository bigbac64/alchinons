 ...
 */
const HexTile = ({ at, position, cell, walkable, color, isHovered, isCurrent, onClick, onHoverChange }) => {
  ...
  return (
    <g transform={`translate(${position.x}, ${position.y})`}>
      <polygon
        points={points}
        fill={color}
        stroke={isCurrent ? '#facc15' : isHovered && walkable ? '#e2e8f0' : 'rgba(15,23,42,0.65)'}
        strokeWidth={isCurrent ? 4 : isHovered && walkable ? 3 : 1.5}
        className={[
          'transition-[stroke,stroke-width,opacity] duration-150 ease-out',
          walkable ? 'cursor-pointer' : 'cursor-not-allowed',
          isHovered && walkable ? 'opacity-95' : 'opacity-100',
        ].join(' ')}
        onClick={() => onClick(at)}
